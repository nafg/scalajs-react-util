...
object FutureValueViewImpl {
  ...
  val component =
    ScalaComponent
      .builder[Props[Option[Try[VdomNode]]]]
      .stateless
      .noBackend
      .render_P {
...
abstract class FutureValueViewLike extends HasBusyIndicator {
  ...
  def custom[A](
    renderLoading: => VdomNode = defaultSettings.renderLoading(),
    onFailure: Throwable => CallbackTo[VdomNode] = defaultSettings.onFailure
  )(content: F[A])(implicit f: A => VdomNode) =
...
abstract class FutureView extends FutureValueViewLike {
  ...
  val component =
    ScalaComponent
      .builder[FutureValueViewImpl.Props[Future[VdomNode]]]
      ...
      .asyncStateFromProps
      .constAlways((p, _, _) => p.content.transform(attempt => Success(Some(attempt))))
      .build
}
